...
function parseCSV(csvText: string): RequestItem[] {
  ...
  return lines.slice(1)
    ...
    .map(line => {
      ...
      return {
        id: `req-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, // Generate unique ID
        ...
        priority: 1 as any, // Default priority (not in CSV)
  ...
  try {
    if (!CSV_URL) {
      return NextResponse.json(
        { error: 'Google Sheets CSV URL not configured' },
        { status: 500 }
      );
    }
